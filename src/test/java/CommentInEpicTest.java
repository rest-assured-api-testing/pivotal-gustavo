...
public class CommentInEpicTest extends ProjectDefault {
    ...
    @BeforeMethod(onlyForGroups={"verifyCommentEpic","deleteCommentEpic","putCommentEpic"})
    public void createCommentToTest() throws JsonProcessingException {
        ...
        ApiRequest apiRequest = baseRequestComment()
                .body(new ObjectMapper().writeValueAsString(commentInEpic))
                .method(ApiMethod.POST).build();
        idCommentToTest = ApiManager.execute(apiRequest).getBody(CommentInEpic.class).getId().toString();
    }

    @AfterMethod(onlyForGroups = {"verifyCommentEpic","putCommentEpic"})
